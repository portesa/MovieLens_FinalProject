...
test_data <- test_data %>% mutate(year = year(rating_dt)) #reate year feature in test set
...
avg_user_rating_test <- test_data %>% group_by(userId) %>% summarize(avg_user_rt=mean(rating)) #avg_user_rating feature in test set
test_data <- test_data %>% inner_join(avg_user_rating_test) #join avg_user_rating to original data in test set
